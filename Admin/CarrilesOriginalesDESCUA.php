...
$cargos = "select Carril, count(*) as Ocupados from dbs9098416.posiciones  where Bodega = $Area and EstatusUbicacion = 'Cuarentena' group by Carril";
...

{
    while ($row = $result->fetch_assoc()) {
        echo '<option value="'.$row['Carril'].'">'.utf8_encode($row['Carril']).' -- '.utf8_encode($row['Ocupados']).'  Ubicaciones Bloqueadas </option>';
    }
}
