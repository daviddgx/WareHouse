...
FROM posiciones  
WHERE Bodega = $Area 
  AND Estado IN ('Ocupada','Ocupada-PK')   
  AND EstatusUbicacion NOT IN ('Calidad','Cuarentena') 
  AND Ubicacion NOT IN (
        SELECT UbicacionOrigen 
        FROM reubicacionesmasivas 
        WHERE estatus = 'Pendiente'
  ) 
GROUP BY Carril
ORDER BY 
    LEFT(Carril, 1),             -- primero por la letra (A, B, C...)
    CAST(SUBSTRING(Carril, 2) AS UNSIGNED);  -- luego por el número
";
