...
include '../LQS_EUQ/LST_PRTSCompletos.php';
...
$sqlDatos = "SELECT * FROM dbs9098416.productos;";
...
if(!$ejecutar_sentencia_Productos)
{
    ...
    if($lista_Productos->num_rows > 0){
        ...
        //set column headers
        $fields = array('IDH', 'CodigoDeBarras', 'Descripcion', 'Marca', 'LINEA', 'UNIDADESXMEDIDA', 'UMEDIDA', 'BASE', 'ALTURA', 'CAJASXPALET', 'PESONETOUNIDAD', 'PESOBRUTOUNIDAD', 'PESONETOCAJA', 'PESOBRUTOCAJA', 'FOTO', 'DIASCUARENTENA', 'DIASVENCIMIENTO', 'ESTADO', 'MINIMOPICKING', 'MAXIMOPICKING');
        fputcsv($f, $fields, $delimiter);

        //output each row of the data, format line as csv and write to file pointer
        while($row = $lista_Productos->fetch_assoc()){

            $lineData = array($row['IDH'], $row['CodigoDeBarras'], $row['Descripcion'], $row['Marca'], $row['LINEA'], $row['UNIDADESXMEDIDA'], $row['UMEDIDA'], $row['BASE'], $row['ALTURA'], $row['CAJASXPALET'], $row['PESONETOUNIDAD'], $row['PESOBRUTOUNIDAD'], $row['PESONETOCAJA'], $row['PESOBRUTOCAJA'], $row['FOTO'], $row['DIASCUARENTENA'], $row['DIASVENCIMIENTO'], $row['ESTADO'], $row['MINIMOPICKING'], $row['MAXIMOPICKING']);
            ;
