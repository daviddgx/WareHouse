...
try {
    ...
    $sqlDestrucciones = "SELECT Ubicacion, Bodega, Carril, Posicion, Nivel, IDH, EstatusProducto, Observaciones FROM posiciones WHERE EstatusUbicacion = 'Destruccion' ORDER BY Bodega, Carril, Posicion, Nivel";
    $resultado = $conn->query($sqlDestrucciones);
    if ($resultado instanceof mysqli_result) {
        while ($fila = $resultado->fetch_assoc()) {
            $destrucciones[] = $fila;
        }
    } else {
        $errores = 'No se pudo obtener el historial de destrucciones.';
    }
} catch (Exception $e) {
        ...
        <div class="container-fluid animate__animated animate__fadeIn">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body ">
                            <h4 class="card-title">Consultar destrucciones</h4>
                            <h6 class="card-subtitle">Revisa el historial y el estado de las destrucciones registradas.</h6>
                            <br>
                            <?php if ($errores !== ''): ?>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                    <?php if (count($destrucciones) === 0): ?>
                                        <tr>
                                            <td colspan="8" class="text-center">No hay destrucciones registradas.</td>
                                        </tr>
                                    <?php else: ?>
                                        <?php foreach ($destrucciones as $registro): ?>
                                            <tr>
                                                <td><?php echo htmlspecialchars($registro['Ubicacion']); ?></td>
                                                <td><?php echo htmlspecialchars($registro['Bodega']); ?></td>
                                                <td><?php echo htmlspecialchars($registro['Carril']); ?></td>
                                                <td><?php echo htmlspecialchars($registro['Posicion']); ?></td>
                                                <td><?php echo htmlspecialchars($registro['Nivel']); ?></td>
                                                <td><?php echo htmlspecialchars($registro['IDH']); ?></td>
                                                <td><?php echo htmlspecialchars($registro['EstatusProducto']); ?></td>
