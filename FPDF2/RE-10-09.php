...
{
   // Cabecera de página
   private $firstPage = true; // Variable para controlar si es la primera página
   ...
   function Header()
   {
      ...
      $this->SetTextColor(103); //color
       ...
       $this->firstPage = false; // Marcamos que ya no es la primera página
      }
...
include '../LQS_EUQ/Connect.php';
$conn  = new PDO('mysql:host='.$servername.';dbname='.$dbname, $username, $password);
$ejecutar_sentencia_Despachos = $conn->query("SELECT Material,b.Descripcion FROM dbs9098416.DetalleGuias_Carga inner join productos b on Material = b.IDH  where Transporte = $NoGuia group by Material;");
$lista_DespachoPRODUCCION =$ejecutar_sentencia_Despachos->fetch(PDO::FETCH_ASSOC);
