...
function DarValorProducciones(){
    ...
    $sentencia = $pdo->prepare("SELECT count(1) as Asignaciones FROM dbs9098416.asignaciones where EstatusProducto = 'PL' and FechaRegistro BETWEEN '".$HoraTrabajoInicio."' and '".$HoraTrabajoFinal."';");
    $sentencia->execute();
...
function DarValorLineas(){
    ...
    $sentencia = $pdo->prepare("select count(DISTINCT(operador)) as operadores from dbs9098416.asignaciones where EstatusProducto = 'PL' and FechaRegistro BETWEEN '".$HoraTrabajoInicio."' and '".$HoraTrabajoFinal."';");
    $sentencia->execute();
    $Count =  $sentencia->fetch(PDO::FETCH_LAZY);
...
function DarValorListaColocadas(){
    ...
    $sentencia = $pdo->prepare("select count(1) as ingresos from dbs9098416.asignaciones where EstatusProducto = 'PL' and Estado = 'Ingresado' and FechaRegistro BETWEEN '".$HoraTrabajoInicio."' and '".$HoraTrabajoFinal."';");
    $sentencia->execute();
    $Count =  $sentencia->fetch(PDO::FETCH_LAZY);
...
function DarValorListaPendientes(){
    ...
    $sentencia = $pdo->prepare("select count(1) as pendientes from dbs9098416.asignaciones where EstatusProducto = 'PL' and Estado = 'Pendiente'  and FechaRegistro BETWEEN '".$HoraTrabajoInicio."' and '".$HoraTrabajoFinal."'; ");
    $sentencia->execute();
    $Count =  $sentencia->fetch(PDO::FETCH_LAZY);
